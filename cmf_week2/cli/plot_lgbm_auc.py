"""LGBM direction ROC/AUC using vendored strategies_bundle."""
...
from cmf_week2.config import FIGURES, RESULTS, STRATEGIES_BUNDLE, has_raw_data
...
def run_symbol(sym: str, max_days: int | None = 90) -> dict:
    sys.path.insert(0, str(STRATEGIES_BUNDLE))
    ...
    fig.tight_layout()
    fig_path = FIGURES / f"lgbm_roc_{sym}_{HORIZON}s.png"
    ...
    return {
        ...
        "auc_test": roc_auc,
        "hit_rate_test": hit,
        "roc_figure": fig_path.name,
    }


def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(description=__doc__)
    p.add_argument("--symbol", default=None, choices=("btcusdt", "ethusdt"))
    args = p.parse_args(argv)

    if not has_raw_data():
        print("ERROR: set LIQUIDATION_DATA_ROOT with raw + enriched parquet", file=sys.stderr)
        return 1

    syms = ("btcusdt", "ethusdt") if args.symbol is None else (args.symbol,)
    rows = [run_symbol(s) for s in syms]
    meta = {
        "model": "lgbm_nk_30s",
        "target": f"y_mu_sign_{HORIZON}s = sign(kalman_mu[t+{HORIZON}s] - kalman_mu[t])",
        "normalization": "train-only z-score, clip ±8",
        "note": "auc_test = ROC-AUC; hit_rate_test = sign agreement (different metrics)",
        "symbols": rows,
    }
    path = RESULTS / "lgbm_direction_metrics.json"
